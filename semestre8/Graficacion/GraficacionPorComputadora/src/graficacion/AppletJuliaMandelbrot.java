...
public class AppletJuliaMandelbrot extends Applet {
 Graphics graphics1;
 boolean julia;
 ...
 public void init() {
  setLayout(null);
  nx=getSize().width+200; 
  ny=getSize().height+200; 
  dpi=nx/3;
  setSize(800,800); // resize?
 ...
 public void generate() {
  ...
  for (int iy=0; iy<=ny; ++iy) {
   ...
   for (int ix=0; ix<=nx; ++ix) {
    if (julia) {x=x1+ix*dx; y=y2-iy*dx;} else {xc=x1+ix*dx; x=0; y=0;}
    unbounded=false;
    for (i=0; (i<imax)&&(!unbounded); ++i) {
     if (power>=2&&power<=25&&power%1==0) { // faster logic
      for (int j=0; j<n; ++j) {t1=x*x-y*y; y=2*x*y; x=t1;}; // z^2
      if (power%2!=0) {t1=x*(x*x-3*y*y); y=y*(3*x*x-y*y); x=t1;} // z^3
     }
     else {t1=Math.atan2(y,x); t2=Math.pow(x*x+y*y,power/2.0); x=t2*Math.cos(power*t1); y=t2*Math.sin(power*t1);} // z^n
...
//   t1=Math.log(x*x+y*y)/2; y=Math.atan(y/x); x=t1; // ln(z)
    ...
    if (unbounded) {
     ...
     if (fcolor==6) {r=(int)(255*t1); g=r; b=r;}
     else {
      c1=(int)Math.min(255*2*t1,255); c2=(int)Math.max(255*(2*t1-1),0);
      switch(fcolor) {
       case 0: r=c1; g=c2; b=c2; break; // red
       case 1: r=c2; g=c1; b=c2; break; // green
       case 2: r=c2; g=c2; b=c1; break; // blue
       case 3: r=c1; g=c1; b=c2; break; // yellow
       case 4: r=c2; g=c1; b=c1; break; // cyan
       case 5: r=c1; g=c2; b=c1; break; // magenta
      }
     }
     pixel(ix,iy,r,g,b);
    }
