...
//      printf("Unidades ASIGNADAS del recurso %d que necesita el proceso %d: ",j+1,i+1);
  ...
  for(k=0; k<nProcesos; k++){
    ...
    for(i=0; i<nProcesos; i++){
      procesoFin[i]=0;
      for(j=0; j<mRecursos; j++){
        if(matrizRecursosNecesarios[i][j] <= recursosDisponibles[j]+matrizRecursosAsignados[i][j] && matrizRecursosMaximos[i][j] <= recursosDisponibles[j]+matrizRecursosAsignados[i][j])
          procesoFin[i]++;
      }
