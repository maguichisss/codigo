...
guardar()
{
    ...
    do
    {
        ...
        if(error<0)
        error=error*-1;
        fprintf(arch,"\n%2.0f  %f  %f  %f  %f",cont,a1,b1,fa,fb);
        if(fxn>0)
            xerr=b1=xn;
        else
            xerr=a1=xn;
        fprintf(arch,"  %f  %f  %f",xn,fxn,error);
    }while(error>tol);
    fprintf(arch,"\n\nLa raiz es: %f\n",xn);
    fclose(arch);
    printf("\n\nGuardado con exito!!!\n\n");
...
metodo()
{
    ...
    do
    {
        ...
        if(error<0)
        error=error*-1;
        printf("\n%2.0f  %f  %f  %f  %f",cont,a,b,fa,fb);
        if(fxn>0)
            xerr=b=xn;
        else
            xerr=a=xn;
        printf("  %f  %f  %f",xn,fxn,error);
    }while(error>tol);
    printf("\n\nLa raiz es: %f\n\n",xn);
    system("pause");
}
